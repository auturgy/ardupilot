 ...
 */
void AP_ExternalAHRS_WIT::send_config(void) const
{
...
*/
static const uint8_t wit_pkt1_header[] { 0x35, 0x03, 0x00, 0x2c, 0x0f, 0x47, 0x01, 0x13, 0x06 };
#define WIT_PKT1_LENGTH 194 // includes header

struct PACKED WIT_packet1 {
...
static_assert(sizeof(WIT_packet1)+2+4*2+2 == WIT_PKT1_LENGTH, "incorrect WIT_packet1 length");
...
*/
static const uint8_t wit_pkt2_header[] { 0x4e, 0x02, 0x00, 0x10, 0x00, 0xb8, 0x20, 0x18, 0x20 };
#define WIT_PKT2_LENGTH 120 // includes header

struct PACKED WIT_packet2 {
    uint64_t timeGPS;
    float temp;
    uint8_t numGPS1Sats;
    uint8_t GPS1Fix;
...
// check packet size for 4 groups
static_assert(sizeof(WIT_packet2)+2+4*2+2 == WIT_PKT2_LENGTH, "incorrect WIT_packet2 length");

// constructor
AP_ExternalAHRS_WIT::AP_ExternalAHRS_WIT(AP_ExternalAHRS *_frontend,
    ...
    AP_ExternalAHRS_backend(_frontend, _state)
{
    ...
    bufsize = MAX(WIT_PKT1_LENGTH, WIT_PKT2_LENGTH);
    pktbuf = new uint8_t[bufsize];
    last_pkt1 = new WIT_packet1;
    last_pkt2 = new WIT_packet2;

    if (!pktbuf || !last_pkt1 || !last_pkt2) {
        AP_HAL::panic("Failed to allocate ExternalAHRS");
    }

    if (!hal.scheduler->thread_create(FUNCTOR_BIND_MEMBER(&AP_ExternalAHRS_WIT::update_thread, void), "AHRS", 2048, AP_HAL::Scheduler::PRIORITY_SPI, 0)) {
 ...
 */
bool AP_ExternalAHRS_WIT::check_uart()
{
    if (!port_opened) {
        return false;
    }
    ...
    match_header1 = (0 == memcmp(&pktbuf[1], wit_pkt1_header, MIN(sizeof(wit_pkt1_header), unsigned(pktoffset-1))));
    match_header2 = (0 == memcmp(&pktbuf[1], wit_pkt2_header, MIN(sizeof(wit_pkt2_header), unsigned(pktoffset-1))));
    if (!match_header1 && !match_header2) {
        goto reset;
    }

    if (match_header1 && pktoffset >= WIT_PKT1_LENGTH) {
        uint16_t crc = crc16_ccitt(&pktbuf[1], WIT_PKT1_LENGTH-1, 0);
        if (crc == 0) {
            // got pkt1
            process_packet1(&pktbuf[sizeof(wit_pkt1_header)+1]);
            memmove(&pktbuf[0], &pktbuf[WIT_PKT1_LENGTH], pktoffset-WIT_PKT1_LENGTH);
            pktoffset -= WIT_PKT1_LENGTH;
        } else {
            goto reset;
        }
    } else if (match_header2 && pktoffset >= WIT_PKT2_LENGTH) {
        uint16_t crc = crc16_ccitt(&pktbuf[1], WIT_PKT2_LENGTH-1, 0);
        if (crc == 0) {
            // got pkt2
            process_packet2(&pktbuf[sizeof(wit_pkt2_header)+1]);
            memmove(&pktbuf[0], &pktbuf[WIT_PKT2_LENGTH], pktoffset-WIT_PKT2_LENGTH);
            pktoffset -= WIT_PKT2_LENGTH;
        } else {
            goto reset;
        }
    }
...
void AP_ExternalAHRS_WIT::update_thread()
{
    if (!port_opened) {
 ...
 */
void AP_ExternalAHRS_WIT::process_packet1(const uint8_t *b)
{
    const struct WIT_packet1 &pkt1 = *(struct WIT_packet1 *)b;
    const struct WIT_packet2 &pkt2 = *last_pkt2;
 ...
 */
void AP_ExternalAHRS_WIT::process_packet2(const uint8_t *b)
{
    const struct WIT_packet2 &pkt2 = *(struct WIT_packet2 *)b;
    const struct WIT_packet1 &pkt1 = *last_pkt1;
...
int8_t AP_ExternalAHRS_WIT::get_port(void) const
{
    if (!uart) {
        return -1;
    }
...
bool AP_ExternalAHRS_WIT::healthy(void) const
{
    uint32_t now = AP_HAL::millis();
    return (now - last_pkt1_ms < 40 && now - last_pkt2_ms < 500);
}

bool AP_ExternalAHRS_WIT::initialised(void) const
{
    return last_pkt1_ms != 0 && last_pkt2_ms != 0;
}

bool AP_ExternalAHRS_WIT::pre_arm_check(char *failure_msg, uint8_t failure_msg_len) const
{
    if (!healthy()) {
        hal.util->snprintf(failure_msg, failure_msg_len, "WIT unhealthy");
        return false;
    }
    if (last_pkt2->GPS1Fix < 3) {
        hal.util->snprintf(failure_msg, failure_msg_len, "WIT no GPS1 lock");
        return false;
    }
    if (last_pkt2->GPS2Fix < 3) {
        hal.util->snprintf(failure_msg, failure_msg_len, "WIT no GPS2 lock");
        return false;
    }
    return true;
}
 ...
 */
void AP_ExternalAHRS_WIT::get_filter_status(nav_filter_status &status) const
{
...
void AP_ExternalAHRS_WIT::send_status_report(mavlink_channel_t chan) const
{
    if (!last_pkt1) {
        return;
    }
    ...
    // send message
    const struct WIT_packet1 &pkt1 = *(struct WIT_packet1 *)last_pkt1;
